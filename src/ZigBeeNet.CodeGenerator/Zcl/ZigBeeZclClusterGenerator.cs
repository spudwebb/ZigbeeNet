...
namespace ZigBeeNet.CodeGenerator.Zcl
{
    public class ZigBeeZclClusterGenerator : ZigBeeBaseClassGenerator
    {
        ...
        private void GenerateZclClusterClasses(ZigBeeXmlCluster cluster, string packageRootPrefix)
        {
            ...
            ImportsAdd("using System.Collections.Concurrent;");
            ImportsAdd("using System.Collections.Generic;");
            ImportsAdd("using System.Linq;");
            ImportsAdd("using System.Text;");
            ImportsAdd("using System.Threading;");
            ImportsAdd("using System.Threading.Tasks;");
            ImportsAdd("using ZigBeeNet.DAO;");
            ...
            if (cluster.Attributes.Count != 0)
            {
                ...
                foreach (ZigBeeXmlAttribute attribute in cluster.Attributes)
                {
                    ...
                    else
                    {
                        ...
                        @out.WriteLine("    public static ushort " + GetEnum(attribute.Name) + " = 0x" + attribute.Code.ToString("X4") + ";");
                    }
                }
                @out.WriteLine();
            }
        ...
        {
            @out.WriteLine("        ConcurrentDictionary<ushort, ZclAttribute> attributeMap = new ConcurrentDictionary<ushort, ZclAttribute>(" + attributes.Count + ");");
            ...
            if (attributes.Count != 0)
            {
                ...
                foreach (ZigBeeXmlAttribute attribute in attributes)
                {
                    ...
                    {
                        @out.WriteLine("        attributeMap.TryAdd(" + GetEnum(attribute.Name) + ", "
