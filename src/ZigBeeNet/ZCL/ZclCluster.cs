...
namespace ZigBeeNet.ZCL
{
    public abstract class ZclCluster
    {
        ...
        /// </summary>
        protected static Dictionary<ushort, Func<ZclCommand>> _genericCommands;
        ...
        */
         ...
         */
        protected Dictionary<ushort, Func<ZclCommand>> InitializeServerCommands()
        {
         ...
         */
        protected Dictionary<ushort, Func<ZclCommand>> InitializeClientCommands()
        {
            return new Dictionary<ushort, Func<ZclCommand>>();
        }
        ...
        public Task<CommandResult> GetReporting(ushort attributeId)
        {
            ...
            command.DestinationAddress = _zigbeeEndpoint.EndpointAddress;
        ...
        public Task<CommandResult> Bind(IeeeAddress address, ushort networkAddress, byte endpointId)
        {
            ...
            {
                SrcAddress = _zigbeeEndpoint.GetIeeeAddress(),
                SrcEndpoint = _zigbeeEndpoint.EndpointId,
                BindCluster = _clusterId,
                DstAddrMode = 3, // 64 bit addressing
                DstAddress = address,
                DestinationAddress = new ZigBeeEndpointAddress(networkAddress, endpointId),
                DstEndpoint = endpointId
            };
        ...
        public Task<CommandResult> Unbind(IeeeAddress address, byte endpointId)
        {
            ...
            {
                DestinationAddress = new ZigBeeEndpointAddress(_zigbeeEndpoint.GetEndpointAddress().Address),
                SrcAddress = _zigbeeEndpoint.GetIeeeAddress(),
                SrcEndpoint = _zigbeeEndpoint.EndpointId,
                BindCluster = _clusterId,
                DstAddrMode = 3, // 64 bit addressing
        ...
        private void UpdateAttribute(ushort attributeId, object attributeValue)
        {
            ...
            if (attribute == null)
            {
                Log.Debug("{EndPoint}: Unknown {IsClient} attribute in {AttributeId} cluster {ClusterId}", _zigbeeEndpoint.EndpointAddress, (_isClient ? "Client" : "Server"), attributeId, _clusterId);
            }
            else
            {
        ...
        public void HandleAttributeStatus(List<ReadAttributeStatusRecord> records)
        {
            foreach (ReadAttributeStatusRecord record in records)
            {
                ...
                {
                    Log.Debug("{EndpointAddress}: Error reading attribute {AttributeIdentifier} in cluster {clusterId} - {Status}", _zigbeeEndpoint.EndpointAddress, record.AttributeIdentifier, _clusterId, record.Status);
                    continue;
                }
